...
def answer(numbers):
    ...
    previous = 0
    while True:
        # is the next number already in the tracker?
        if numbers[i] not in tracker:
            # if not, add to tracker
            tracker.append(numbers[i])
        # if already in tracker:
        else:
            # the loop is from first appearance to this appeance
            loop_start = tracker.index(numbers[i])
            # slice the list
            tracker = tracker[loop_start:previous+1]
