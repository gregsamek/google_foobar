...
def answer(minions):
	...
	for i in range(minion_count):
		true_count.append([i, 0])

	for i in range(minion_count):
		for j in range(minion_count):
			test = compare(minions[i], minions[j])
			if test:
				true_count[i][1] += 1
	
	true_count.sort(key=lambda x: int(x[1]), reverse=True)
	result = []
	for each in true_count:
...
def compare(a, b):
	if a == b:
		return None
	top = a[0] * (1+ (1.0 * b[1] - b[2]) / b[2])
	bot = b[0] * (1+ (1.0 * a[1] - a[2]) / a[2])
	if top / bot <= 1:
		return True # that a comes before b
	else:
		return False # that a comes before b (b comes first)
