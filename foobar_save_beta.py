...
def memoize(f):
    memo = {}
    def helper(left_overs, i, j):
        t = (left_overs, i, j)
        if t not in memo:            
            memo[t] = f(left_overs, i, j)
        return memo[t]
    return helper

def answer(food, grid):
    @memoize
    def nom_nom(left_overs, i, j):
        left_overs -= grid[i][j]
        if i < 0 or j < 0 or left_overs < 0:
            return 200
        elif i == j == 0:
            return left_overs
        else:
            return min(nom_nom(left_overs, i - 1, j), nom_nom(left_overs, i, j - 1))

    result = nom_nom(food, len(grid) - 1, len(grid) - 1)
    if result <= food:
        return result
    else:
        return -1
